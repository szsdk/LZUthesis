...
\dirtree{%
    ...
    .2 caspervector\DTcomment{参考文献格式配置}.
    .3 biblatex-caspervector-gbk.def.
    .3 biblatex-caspervector-utf8.def.
    .3 caspervector.bbx.
    .3 caspervector.cbx.
}
\section{参考文献}
参考文献的格式是按照GB/T 7714-2005标准的。需要三个额外的格式配置文件：\filename{biblatex-caspervector-gbk.def}、\filename{biblatex-caspervector-utf8.def}、\filename{caspervector.cbx}和\filename{caspervector.bbx}。调用的方法为在导言区加入：
\begin{minted}{latex}
\usepackage[
    backend = biber, style = caspervector, utf8,
    giveninits = true, sortgiveninits = true
