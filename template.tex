...
\end{note}
这个版本在被放出的时候就注定被抛弃了。因为新版的\package{biblatex}（v3.4）中，已经不支持这里所用的参考文献模板，所以在新版本中将会使用\package{caspervector}这个参考文献模板。
...
\section{参考文献}
参考文献的格式是按照GB/T 7714-2005标准的。调用的方法为在导言区加入：
\begin{minted}{latex}
\usepackage[
    backend = biber, style = caspervector, utf8,
    giveninits = true, sortgiveninits = true
